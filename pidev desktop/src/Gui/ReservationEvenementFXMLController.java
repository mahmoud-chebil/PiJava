...
public class ReservationEvenementFXMLController implements Initializable {
    ...
    EvenementService Seven=new EvenementService();
    ...
    public void initialize(URL url, ResourceBundle rb) {
       ...
       //remplir le combobox avec les description des évènements disponibles
        try {
            ...
            ResultSet rs = cnx.createStatement().executeQuery("SELECT id,decription_evenement from `evenement`");
            while(rs.next()){
                options.add(rs.getString("id")+":"+rs.getString("decription_evenement"));
            }
    ...
    private void ajouter(ActionEvent event) throws IOException{
        ...
        Reservation res = new Reservation(0,Integer.parseInt(nbpers.getText()),Date.valueOf(LocalDate.now()),0,idev,1,0);
        try {
