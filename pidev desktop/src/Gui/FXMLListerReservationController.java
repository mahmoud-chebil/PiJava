...
public class FXMLListerReservationController implements Initializable {
   ...
   Reservation r;
    ...
    public void initialize(URL url, ResourceBundle rb) {
       ...
       ResultSet rs = cnx.createStatement().executeQuery("SELECT r.id,u.username,e.decription_evenement,r.dateres,r.nb_personne,e.nbre_places,r.etat from evenement e, reservation r ,user u where r.even_id=e.id and r.user_id=u.id");
      ...
      if(treserv.getSelectionModel().getSelectedIndex()==-1)
       ...
       try{
           ...
           desceven=selectedRes.getDesceven();
            ResultSet rst = cnx.createStatement().executeQuery("SELECT nbre_places from `evenement` where decription_evenement='"+desceven+"'");
             if (rst.next())
