...
//        String query = "SELECT * FROM reclamation";
        ...
        try {
            ...
            ResultSet s = cnx.createStatement().executeQuery("SELECT * from `type_reclamation` ");
            while (s.next()) {
            ...
            ResultSet s1 = cnx.createStatement().executeQuery("SELECT * from `user` ");
            while (s1.next()) {
                user.getItems().add(new User(s1.getInt("id"), s1.getString("username")));
                
                 }
            
            ResultSet rs2 = cnx.createStatement().executeQuery("SELECT id from user where username='"+options1+"'");
            int id;
            if (rs2.next())
                id=rs2.getInt(1);
